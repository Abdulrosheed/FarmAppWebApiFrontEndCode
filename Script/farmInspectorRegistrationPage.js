...
const registerUserRequest = () => {
   ...
   console.log(token)
    ...
    fetch('https://localhost:5001/api/FarmInspector/RegisterFarmInspector', {
        method: 'POST',
        body: dd,
        headers : {
...
const validateInputs = (userNameInput,emailInput,lastNameInput,
    firstNameInput,countryInput,stateInput,localGovernmentInput,genderInput,phonenumber) => {
    return userNameInput.value.trim() == ""  || emailInput.value.trim() == "" ||  lastNameInput.value.trim() == ""
...
registerInputSubmitBtn.addEventListener('click', function(e)  {
    e.preventDefault();
    if (validateInputs(registerUserName,registerEmail, registerInputLastName,registerInputFirstName,countryselectTag,stateSelectTag,
