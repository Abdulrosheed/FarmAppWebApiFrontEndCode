...
const adminRegisterUserRequest = () => {
     ...
     console.log(registerInputPassWord.value);
    let data = 
    {
    ...
    fetch('https://localhost:5001/api/FarmInspector/UpdateFarmInspector', {
        method: 'PUT',
...
function GetElementById()
{
   ...
   let token = localStorage.getItem("farmInspector")
   console.log(token) 
    fetch("https://localhost:5001/api/FarmInspector/GetFarmInspector/",
    {
        headers : {
            "Authorization" : 'Bearer ' + token,
            'Content-Type': 'application/json'
